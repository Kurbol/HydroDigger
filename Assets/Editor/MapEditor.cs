    ...
    public void OnSceneGUI()
    {
        ...
        if (Event.current.type == EventType.MouseDown || Event.current.type == EventType.MouseDrag)
        {
            if (MapManager.Map != null)
            {
                ...
                Coordinate coordinateToDraw = MapManager.Map.GetCoordinateFromPosition(sceneMousePosition);

                MapManager.Map.Draw(coordinateToDraw, mapGenerator.drawingSoilType, mapGenerator.drawRadius);
